...
def newestquery(distro, queryfilter, join=None):
    if join is None:
        return dbs[distro].session.query(Packages[distro], Distribution[distro].repo, func.max(Packages[distro].Date)).join(Distribution[distro]).filter(queryfilter).group_by(Packages[distro].Name, Distribution[distro].repo).order_by(desc(Packages[distro].Date)).all()
    else:
        return dbs[distro].session.query(Packages[distro], Distribution[distro].repo, func.max(Packages[distro].Date)).join(Distribution[distro]).outerjoin(join).filter(queryfilter).group_by(Packages[distro].Name, Distribution[distro].repo).order_by(desc(Packages[distro].Date)).all()
